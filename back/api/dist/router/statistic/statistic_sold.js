...
router.post("/sold", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (data.type == 1 && data.status == 2) {
            ...
            let ticket_multi_use = account.data.UserNonce[0].ticket_multi_use;
            let money_total_ticket_ot = account.data.UserNonce[0].money_total_ticket_ot;
            let money_total_ticket_mul = account.data.UserNonce[0].money_total_ticket_mul;
            let ticket_sold_event = event.data.Event[0].ticket_sold == null
                ? 0 + 1
                : event.data.Event[0].ticket_sold + 1;
            let total_proceed = event.data.Event[0].total_proceed == null
                ? ticket.data.TicketTokens[0].price
                : event.data.Event[0].total_proceed +
                    ticket.data.TicketTokens[0].price;
            let data_update_event = yield update_event({ id: ticket.data.TicketTokens[0].event, input: {
            ...
            const ticket_sold = account.data.UserNonce[0].ticket_sold + 1;
